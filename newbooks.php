...
$sendto = $_POST ['sendto'] ;
$school = $_POST ['school'] ;
If ($sendto  == "หน่วยงาน" and $school=="") {//ถ้ามีการเลือกส่งระบุหน่วยงานให้ใช้ค่าจากในตัวแปร $school
echo "<center><h2>กรุณาระบุชื่อหน่วยงาน</h2> <br><input type=\"button\" value=\"&nbsp;แก้ไข&nbsp;\" onClick=\"javascript:history.go(-1)\" ></center>" ;
exit () ;
} elseif ($sendto  == "หน่วยงาน" and $school<>"") {
	$sendto= $school ;
}
...
include "connect.php" ;
$sql = "select * from $tablenewbooks  Order By id DESC" ;
...
if ($bookfile <> "") { 
 ...
 if ($lastname =="doc" or $lastname =="pdf" or $lastname =="xls" or $lastname =="zip" or $lastname =="jpg" or $lastname =="gif" ) {
	 ...
	 copy ($bookfile, "booksfile/".$filename)  ; 
include "connect.php" ;

 $sql= "INSERT INTO $tablenewbooks (`id`,`sendfrom`,`sendto`,`subject`,`message`,`reply`, `sendtimestamp`, `timestamp24`, `bookfile`,`nview`,`nreply`) VALUES ('$id', '$from', '$sendto', '$subject', '$message', '$reply', '$sendtimestamp', '$timestamp24', '$filename','','') ";
$result= mysql_db_query ($dbname, $sql); 
If ($result) {
