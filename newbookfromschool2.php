...
include "connect.php" ;
$sql = "select * from $tablenewbooksfromschool2  Order By id DESC" ;
...
mysql_close () ;
 ...
 if ($lastname =="pdf" or $lastname =="xls" or $lastname =="zip" or $lastname =="jpg" or $lastname =="gif" ) {
	 ...
	 copy ($bookfile, "booksfilefromschool2/".$filename)  ; 
include "connect.php" ;

 $sql= "INSERT INTO $tablenewbooksfromschool2 (`id`,`fromname`,`subject`, `sendtimestamp`, `bookno`,`bookfile`, `nreply`) VALUES ('$id', '$fromname', '$subject', '$sendtimestamp', '$bookno' , '$filename', '') ";
$result= mysql_db_query ($dbname, $sql); 
If ($result) {
	echo" <center><h2>บันทึกข้อมูลเรียบร้อยแล้ว</h2> <br> <input type=\"button\" value=\"กลับหน้าหลัก\" onClick=\"window.location='index2.php'\">&nbsp;<input type=\"button\" value=\"ส่งหนังสืออีก\" onClick=\"window.location='form_bookfromschool2.php'\">&nbsp;<input type=\"button\" value=\"ดูผลการส่ง\" onClick=\"window.location='bookfromschool2.php'\"></center> ";
} else{
