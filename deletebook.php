...
$search="SELECT  *  FROM $tablenewbooks where sendfrom='$school' " ;
}
...
<TITLE>ระบบงานสารบรรณ<? echo _NAME ;?></TITLE>
...
<TABLE border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1" height="108">
  ...
  <TR>
    ...
    <span style="font-size: 13pt; font-weight: 700">ลบหนังสือที่ส่งโดย <?=$school?></span></font></TD>
  </TR>
  <TR>
     <TD width="71%" height="16"align="right">
          <input type="button" value="กลับหน้าหลัก" onClick="window.location='index.php'" >
&nbsp;<input type="button" value="โหลดหน้านี้ใหม่" onClick="window.location='deletebook.php'" ></TD>
  </TR>
</TABLE>
<TABLE border="1" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#C0C0C0" width="100%" id="AutoNumber2" height="20">
  <TR>
    ...
    <FONT color="#993366" size="2"><B>อ่าน<span lang="en-us">/</span>ตอบ</B></FONT></TD>
...
<tr  bgcolor="<?=$color?>">
    ...
    <TD width="28%" height="20">&nbsp;<A HREF="bookviewsave.php?id=<?=$id?>"  title='คลิกเพื่อเปิดอ่านหนังสือ' style="text-decoration: none" target="_blank"><?=$subject?></A>&nbsp;<? if ($bookfile<>"") {echo"<IMG SRC=image/file.gif WIDTH=13 HEIGHT=10 BORDER=0 ALT='มีไฟล์แนบ'>" ; }?></TD>
    <TD width="23%" height="20">&nbsp;<FONT SIZE="" COLOR="#6600CC"><?=$sendto?></FONT></TD>
    <TD width="17%" height="20">&nbsp;<FONT SIZE="" COLOR="#CC00CC"><?=$from?></FONT></TD>
    <TD width="15%" height="20"><CENTER><FONT SIZE="" COLOR="#6600CC"><?=$send_d?>&nbsp;<?=$send_m?>&nbsp;<?=$send_y?></FONT>&nbsp;/&nbsp;<FONT SIZE="" COLOR="#990099"><?=$send_t?>&nbsp;น.</FONT></CENTER></TD>
    <TD width="6%" height="20"><CENTER><B><FONT SIZE="" COLOR="#FF0033"><?=$nview?></FONT>/<FONT SIZE="" COLOR="#00CC00"><?=$nreply ?></FONT></B></CENTER></TD>
    <TD width="3%" height="20"><CENTER><FONT SIZE="" COLOR="#009900"><A HREF="deletebooks_result.php?id=<?=$id?>" title="คลิกเพื่อลบหนังสือฉบับนี้" onclick="return confirm ('ต้องการลบหนังสือลำดับที่ <?=$bookid?> ออกจากระบบ ?')"><img src="image/delete.png" border=0></A></FONT></CENTER></TD>
  </TR>
  <?
			$list++;
}
...
for ($i=1; $i<=$totalpage; $i++) {
	if ($i == $pageid) {
		...
		echo "[<a href=\"deletebook.php?pageid=$i&keyword=$keyword\">$i</a>]&nbsp;";
	}
 }
 ?>
 <center>
