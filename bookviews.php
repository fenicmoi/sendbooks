...
If ($bookfile<>"") {
		header ("Location:booksfile/$bookfile") ;
	} else {
...
//exit() ;
include "connect.php" ;
$sqlselect = "select * from $tableanswer where bookid='$id' and name='$from' " ;
$resultselect = mysql_db_query($dbname, $sqlselect) ;
$num = mysql_num_rows ($resultselect) ;
if ($num==0){

$sql= "INSERT INTO $tableanswer  VALUES (' ', '$id', '$from', '$answer', '$anstimestamp', '$ans_m', '$ans_y', '$score', '$ip') ";
$result= mysql_db_query ($dbname, $sql); 
mysql_close () ;
If ($result) {
...
//echo"$nview" ;
//exit();
		$sql2 = "UPDATE $tablenewbooks set nreply='$nreply' where id='$id' " ;
