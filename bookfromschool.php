...
if ($keyword<>"") {
$search="SELECT  *  FROM $tablenewbooksfromschool where subject like '%$keyword%' or fromname like '%$keyword%'" ;
} else {
$search="SELECT *  FROM $tablenewbooksfromschool" ;
}
...
<TABLE border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" id="AutoNumber1" height="108">
  ...
  <TR>
    ...
    <span style="font-size: 13pt; font-weight: 700"> (ทะเบียนหนังสือส่งจากโรงเรียน มีหนังสือทั้งหมด <?=$totalrecordall?> ฉบับ)</span></font></TD>
  </TR>
  <TR>
     <TD width="71%" height="16"align="right">
          <input type="button" value="กลับหน้าหลัก" onClick="window.location='index.php'" >
&nbsp;<input type="button" value="โหลดหน้านี้ใหม่" onClick="window.location='bookfromschool.php'" ></TD><FORM METHOD=POST ACTION="<?=$PHP_SELF?>"><TD width="29%" height="16"align="right">
...
<tr  bgcolor="<?=$color?>">
    ...
    <TD width="28%" height="20">&nbsp;<FONT SIZE="" COLOR="#6600CC"><? if ($from == $school or $status == "Yes1" or $status == "Yes") { ?><A HREF="download.php?id=<?=$id?>"  title='คลิกเพื่อเปิดดาวน์โหลดหนังสือ' style="text-decoration: none" target="_blank"><?=$subject?></A><?} else {?><?=$subject?> <?}?>&nbsp;<? if ($bookfile<>"") {echo"<IMG SRC=image/file.gif WIDTH=13 HEIGHT=10 BORDER=0 ALT='มีไฟล์แนบ'>" ; }?></FONT></TD>
    ...
    <?if ($status == "Yes1") { ?> <TD width="3%" height="20"><CENTER><FONT SIZE="" COLOR="#009900"><A HREF="deletebookfromschool.php?id=<?=$id?>" title="คลิกเพื่อลบหนังสือฉบับนี้" onclick="return confirm ('ต้องการลบหนังสือลำดับที่ <?=$bookid?> ออกจากระบบ ?')">
...
<hr>
<FONT SIZE="" COLOR="#CC00CC"><B>แสดงหนังสือ <?=$totalrecord?> ฉบับล่าสุด จากจำนวนหนังสือทั้งหมด <?=$totalrecordall?> ฉบับ </B></FONT><FONT SIZE="" COLOR="#6600CC">(แต่ท่านสามารถสืบค้นหนังสือทั้งหมดได้จากช่องค้นหาหนังสือด้านบน)</FONT><BR>
<span lang="th"><b><font face="MS Sans Serif" color="#800080">หน้าละ  <?=$pagesize?> รายการ</font><font face="MS Sans Serif" color="#808000"> </font></b></span><b>
...
for ($i=1; $i<=$totalpage; $i++) {
	if ($i == $pageid) {
		...
		echo "[<a href=\"bookfromschool.php?pageid=$i&keyword=$keyword\">$i</a>]&nbsp;";
	}
 }
 ?>
 <center>
